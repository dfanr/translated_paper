...
| 【概述】原文 | 【概述】翻译 |
| ---- | ---- |
| ✅ Automated visual understanding of our diverse and open world demands computer vision models to generalize well with minimal customization for specific tasks, similar to human vision. Computer vision foundation models, which are trained on diverse, large-scale dataset and can be adapted to a wide range of downstream tasks, are critical for this mission to solve real-world computer vision applications. While existing vision foundation models such as CLIP (Radford et al., 2021), ALIGN (Jia et al., 2021), and Wu Dao 2.0 (Wud, ) focus mainly on mapping images and textual representations to a cross-modal shared representation, we introduce a new computer vision foundation model, Florence, to expand the representations from coarse (scene) to fine (object), from static (images) to dynamic (videos), and from RGB to multiple modalities (caption, depth). By incorporating universal visual-language representations from Web-scale image-text data, our Florence model can be easily adapted for various computer vision tasks, such as classification, retrieval, object detection, VQA, image caption, video retrieval and action recognition. Moreover, Florence demonstrates outstanding performance in many types of transfer learning: fully sampled fine-tuning, linear probing, few-shot transfer and zero-shot transfer for novel images and objects. All of these properties are critical for our vision foundation model to serve general purpose vision tasks. Florence achieves new state-of-the-art results in majority of 444444 representative benchmarks, e.g. ImageNet-1K zero-shot classification with top-1 accuracy of 83.7483.74{83.74} and the top-5 accuracy of 97.1897.18{97.18}, 62.462.4{62.4} mAP on COCO fine tuning, 80.3680.36{80.36} on VQA, and 87.887.8{87.8} on Kinetics-600. | ✅ Automated visual understanding of our diverse and open world demands computer vision models to generalize well with minimal customization for specific tasks, similar to human vision. Computer vision foundation models, which are trained on diverse, large-scale dataset and can be adapted to a wide range of downstream tasks, are critical for this mission to solve real-world computer vision applications. While existing vision foundation models such as CLIP (Radford et al., 2021), ALIGN (Jia et al., 2021), and Wu Dao 2.0 (Wud, ) focus mainly on mapping images and textual representations to a cross-modal shared representation, we introduce a new computer vision foundation model, Florence, to expand the representations from coarse (scene) to fine (object), from static (images) to dynamic (videos), and from RGB to multiple modalities (caption, depth). By incorporating universal visual-language representations from Web-scale image-text data, our Florence model can be easily adapted for various computer vision tasks, such as classification, retrieval, object detection, VQA, image caption, video retrieval and action recognition. Moreover, Florence demonstrates outstanding performance in many types of transfer learning: fully sampled fine-tuning, linear probing, few-shot transfer and zero-shot transfer for novel images and objects. All of these properties are critical for our vision foundation model to serve general purpose vision tasks. Florence achieves new state-of-the-art results in majority of 444444 representative benchmarks, e.g. ImageNet-1K zero-shot classification with top-1 accuracy of 83.7483.74{83.74} and the top-5 accuracy of 97.1897.18{97.18}, 62.462.4{62.4} mAP on COCO fine tuning, 80.3680.36{80.36} on VQA, and 87.887.8{87.8} on Kinetics-600. |
...
| 【图标题】原文 | 【图标题】翻译 |
| ---- | ---- |
| ✅ Figure 1:  Common computer vision tasks are mapped to a Space-Time-Modality space. | ✅ Figure 1:  常见的计算机视觉任务被映射到Space-Time-Modality空间。 |
...
| ---- | ---- |
| ✅ We raise the question: “ What is the foundation model for computer vision ?”. | ✅ 我们提出问题：“What is the foundation model for computer vision？”。 |
| ✅ But first, in order to better define what “foundation” means in computer vision, we capture the spectrum of tasks in a problem space (Figure 1 ) with three orthogonal axes: 1) Space : from coarse ( e.g.  scene-level classification) to fine-grained ( e.g.  object detection), 2) Time : from static ( e.g.  images) to dynamic ( e.g.  videos), and 3) Modality : from RGB only to multiple senses ( e.g.  captioning and depth). | ✅ 但首先，为了更好地定义“基础”在计算机视觉中的含义，我们用三个正交轴捕获问题空间中的任务范围（图 1）：1）Space：从粗粒度（e.g.  场景级分类）到细粒度（e.g.  物体检测），2）Time：从静态（e.g.  图像）到动态（e.g.  视频），3）Modality：从仅 RGB 到多种感官（e.g.  字幕和深度）。 |
| ✅ Due to the diversity nature of visual understanding, we redefine foundation models for computer vision to be a pre-trained model and its adapters for solving all vision tasks in this Space-Time-Modality space, with transferability such as zero-/few-shot learning and fully fine tuning, etc. | ✅ 由于视觉理解的多样性，我们将 foundation models for computer vision 重新定义为 a pre-trained model and its adapters，用于解决该时空模态空间中的所有视觉任务，具有零次/小样本学习和完全微调等可转移性。 |
...
| ---- | ---- |
| ✅ In this paper, we present an emerging paradigm for building a vision foundation model , called Florence. | ✅ 在本文中，我们提出了一个用于构建 vision foundation model 的新兴范例，称为 Florence。 |
| ✅ We use the name of Florence as the origin of the trail for exploring vision foundation models, as well as the birthplace of Renaissance. | ✅ 我们以Florence这个名字，作为探寻vision foundation车型的轨迹的起源，也是文艺复兴的发源地。 |
| ✅ Florence is trained on noisy Web-scale data end-to-end with a unifying objective, allowing the model to achieve best-in-class performance across a wide range of benchmarks. | ✅ Florence 采用统一目标，在嘈杂的 Web 规模数据上进行端到端训练，从而使模型在广泛的基准测试中实现一流的性能。 |

| 【第1节，第6段】原文 | 【第1节，第6段】翻译 |
| ---- | ---- |
| ✅ The ecosystem of constructing Florence consists of data curation , model pretraining , task adaptations and training infrascturue , as shown in Figure 2 . | ✅ 构建Florence的生态系统由data curation、model pretraining、task adaptations、training infrascturue组成，如图2所示。 |
...
| ✅ As Web-crawled data is usually noisy free-form texts ( e.g.  , word, phrase or sentence), to attain more effective learning, we consider UniCL , a unified image-text contrastive learning objective recently proposed in ( **Unified contrastive learning in image-text-label space.** ) , which has demonstrated improvements over contrastive and supervised learning approaches. | ✅ 由于网络爬取的数据通常是嘈杂的自由格式文本（e.g. ，单词，短语或句子），为了实现更有效的学习，我们考虑使用UniCL，这是最近在( **Unified contrastive learning in image-text-label space.** )中提出的统一的图像文本对比学习目标，它已经证明了对比和监督学习方法的改进。 |

| 【第1节，第8段】原文 | 【第1节，第8段】翻译 |
| ---- | ---- |
| ✅ Model pretraining (representation learning). | ✅ Model pretraining（表示学习）。 |
| ✅ To learn a good representation from image-text pairs, we used a two-tower architecture including an image encoder and a language encoder, as commonly used in CLIP ( **Learning transferable visual models from natural language supervision.** ) and ALIGN ( **Scaling up visual and vision-language representation learning with noisy text supervision.** ). | ✅ 为了从图像-文本对中学习良好的表示，我们使用了双塔 architecture，其中包括一个图像编码器和一个语言编码器，就像在 CLIP ( **Learning transferable visual models from natural language supervision.** ) 和 ALIGN ( **Scaling up visual and vision-language representation learning with noisy text supervision.** ) 中常用的那样。 |
| ✅ For the image encoder, we chose hierarchical Vision Transformers ( e.g.  , Swin ( **Swin transformer: Hierarchical vision transformer using shifted windows.** ) , CvT ( **Cvt: Introducing convolutions to vision transformers.** ) , Vision Longformer ( **Multi-scale vision longformer: A new vision transformer for high-resolution image encoding.** ) , Focal Transformer ( **Focal self-attention for local-global interactions in vision transformers.** ) , and CSwin ( **Cswin transformer: A general vision transformer backbone with cross-shaped windows.** ) ). | ✅ 对于图像编码器，我们选择了分层 Vision Transformers（e.g. 、Swin ( **Swin transformer: Hierarchical vision transformer using shifted windows.** )、CvT ( **Cvt: Introducing convolutions to vision transformers.** )、Vision Longformer ( **Multi-scale vision longformer: A new vision transformer for high-resolution image encoding.** )、Focal Transformer ( **Focal self-attention for local-global interactions in vision transformers.** ) 和 CSwin ( **Cswin transformer: A general vision transformer backbone with cross-shaped windows.** )）。 |
...
| ✅ As we have defined computer vision foundation models to adapt to various downstream tasks, it is vital for Florence  to be extensible and transferable for this purpose. | ✅ 由于我们已经定义了计算机视觉基础模型来适应各种下游任务，因此 Florence  成为 extensible 和 transferable 至关重要。 |
| ✅ We extended the learned feature representation along space (from scene to objects) using the dynamic head adapter ( **Dynamic head: Unifying object detection heads with attentions.** ) , time (from static image to videos) via proposed video CoSwin adapter, and modality (from images to language) via METER adapter ( **An empirical study of training end-to-end vision-and-language transformers.** ). | ✅ 我们使用动态头适配器 ( **Dynamic head: Unifying object detection heads with attentions.** ) 沿空间（从场景到物体）扩展了学习到的特征表示，通过提出的视频 CoSwin 适配器沿时间（从静态图像到视频）扩展了学习到的特征表示，并通过 METER 适配器 ( **An empirical study of training end-to-end vision-and-language transformers.** ) 沿模态（从图像到语言）扩展了学习到的特征表示。 |
| ✅ Florence  is designed to effectively adapted in the open world via few-shot and zero-shot transfer learning, with the ability of efficient deployment by extra training with few epochs ( e.g.  in retrieval). | ✅ Florence  旨在通过少样本和零样本迁移学习有效适应开放世界，并能够通过少量阶段的额外训练实现有效部署（e.g.  用于检索）。 |
...
| ✅ Given an image-text pair, we generate a triplet  $(\boldsymbol{x},\boldsymbol{t},\boldsymbol{y})$  via a text hash-table, where  $\boldsymbol{x}$  is the image,  $\boldsymbol{t}$  is the language description ( i.e.  , hash value), and  $\boldsymbol{y}$  is the language label ( i.e.  , hash key) indicating the index of unique language description in the dataset. | ✅ 给定一个图像-文本对，我们通过文本哈希表生成一个三元组  $(\boldsymbol{x},\boldsymbol{t},\boldsymbol{y})$ ，其中  $\boldsymbol{x}$  是图像， $\boldsymbol{t}$  是语言描述（i.e. ，哈希值）， $\boldsymbol{y}$  是语言标签（i.e. ，哈希键），表示数据集中唯一语言描述的索引。 |
| ✅ Note that we only map identical language description to the same hash key, i.e.  , language label. | ✅ 请注意，我们只将相同的语言描述映射到相同的哈希键 i.e. （语言标签）。 |
...
| ✅ The unified learning objective in the common image-label-description space unifies two popular learning paradigms – mapping images to the label for learning discriminative representations ( i.e.  , supervised learning) and assigning each description with a unique label for language-image pre-training ( i.e.  , contrastive learning). | ✅ 通用图像-标签-描述空间中的统一学习目标统一了两种流行的学习范式——将图像映射到标签以学习判别性表示（i.e. ，监督学习）并为每个描述分配一个唯一的标签以进行语言图像预训练（i.e. ，对比学习）。 |

| 【第2.2节，第3段】原文 | 【第2.2节，第3段】翻译 |
| ---- | ---- |
| ✅ Our empirical experiments indicate that long language descriptions with rich content would be more beneficial for image-text representation learning than short descriptions ( e.g.  , one or two words). | ✅ 我们的实证实验表明，内容丰富的长语言描述比短描述（e.g. ，一两个单词）更有利于图像文本表示学习。 |
...
| ✅ During training, we randomly select one template to generate  $\boldsymbol{t}$  for each short language description. | ✅ 在训练过程中，我们为每个简短语言描述随机选择一个模板来生成 $\boldsymbol{t}$ 。 |
...
| ---- | ---- |
| ✅ Our Florence pretrained model uses a two-tower architecture: a 12-layer transformer ( **Attention is all you need.** ) as language encoder, similar to CLIP ( **Learning transferable visual models from natural language supervision.** ) , and a hierarchical Vision Transformer as the image encoder. | ✅ 我们的 Florence 预训练模型采用双塔架构：12 层变压器 ( **Attention is all you need.** ) 作为语言编码器，类似于 CLIP ( **Learning transferable visual models from natural language supervision.** )，以及分层 Vision Transformer 作为图像编码器。 |
| ✅ The hierarchical Vision Transformer is a modified Swin Transformer ( **Swin transformer: Hierarchical vision transformer using shifted windows.** ) with convolutional embedding, called CoSwin Transformer. | ✅ 分层 Vision Transformer 是具有卷积嵌入的改进版 Swin Transformer ( **Swin transformer: Hierarchical vision transformer using shifted windows.** )，称为 CoSwin Transformer。 |
...
| ---- | ---- |
| ✅ We extend the Florence pretrained model to learn fine-grained ( i.e.  , object-level) representation, which is fundamental to dense prediction tasks such as object detection. | ✅ 我们扩展了 Florence 预训练模型来学习细粒度（i.e. ，对象级）表示，这对于对象检测等密集预测任务至关重要。 |
| ✅ For this goal, we add an adaptor Dynamic Head   ( **Dynamic head: Unifying object detection heads with attentions.** ) (or Dynamic DETR ( **Dynamic detr: End-to-end object detection with dynamic attention.** ) ), a unified attention mechanism for the detection head, to the pretrained image encoder ( i.e.  , CoSwin ). | ✅ 为了实现这一目标，我们在预训练图像编码器（i.e. 、CoSwin）中添加了一个适配器 Dynamic Head   ( **Dynamic head: Unifying object detection heads with attentions.** )（或动态 DETR ( **Dynamic detr: End-to-end object detection with dynamic attention.** )），即检测头的统一注意机制。 |
...
| ✅ The key idea of Dynamic Head   ( **Dynamic head: Unifying object detection heads with attentions.** ) is to deploy three attention mechanisms, each on one of the orthogonal dimensions of the tensor, i.e.  , level-wise, spatial-wise, and channel-wise. | ✅ Dynamic Head   ( **Dynamic head: Unifying object detection heads with attentions.** ) 的关键思想是在张量的正交维度之一上部署三种注意力机制，i.e.  ，即逐层、逐空间和逐通道。 |
...
| ✅ Figure 3 shows the Dynamic Head building blocks. | ✅ 图 3 显示了 Dynamic Head 构建块。 |
...
| ---- | ---- |
| ✅ We have constructed a large-scale object detection dataset, called FLOD-9M (for FL orence O bject detection D ataset), for object detection pre-training. | ✅ 我们构建了一个大规模物体检测数据集，称为 FLOD-9M（用于 FL 或 O 物体检测 D 数据集），用于物体检测预训练。 |
...
| ✅ We then pre-train our Dynamic Head model for  $12$  epochs with batch size  $128$  , which takes  $7$  days on  $128$  NVIDIA-A100 GPUs. | ✅ 然后，我们对 Dynamic Head 模型进行预训练，以适应  $12$  时期，批次大小为  $128$ ，这在  $128$  NVIDIA-A100 GPU 上需要  $7$  天。 |
...
| ✅ In the vision-language area, e.g.  visual question answering (VQA) and image captioning, fine-grained representation ( i.e.  , object-level) is indispensable. | ✅ 在视觉语言领域，e.g.  视觉问答（VQA）和图像字幕，细粒度表示（i.e. ，对象级）不可或缺。 |
...
| ---- | ---- |
| ✅ In the Florence V+L adaptation model, we replace the image encoder of METER ( **An empirical study of training end-to-end vision-and-language transformers.** ) with Florence pretrained model CoSwin , and use a pretrained Roberta ( **RoBERTa: A robustly optimized bert pretraining approach.** ) as the language encoder, shown in Figure 4. | ✅ 在 Florence V+L 适配模型中，我们用 Florence 预训练模型 CoSwin 替换 METER ( **An empirical study of training end-to-end vision-and-language transformers.** ) 的图像编码器，并使用预训练的 Roberta ( **RoBERTa: A robustly optimized bert pretraining approach.** ) 作为语言编码器，如图 4 所示。 |
| ✅ The Florence pretrained language encoder can be used for this adapter as it utilizes BERT-based architecture. | ✅ Florence 预训练语言编码器可用于此适配器，因为它采用基于 BERT 的架构。 |
...
| ✅ Our Video CoSwin adapter can borrow the image encoder from CoSwin for the video domain with minimum changes, similar to prior work ( **Video swin transformer.** ). | ✅ 我们的视频 CoSwin 适配器可以借用 CoSwin 中的图像编码器用于视频域，只需进行最少的更改，类似于之前的工作 ( **Video swin transformer.** )。 |
| ✅ First, the image tokenization layer is replaced with a video tokenization layer. | ✅ 首先，将图像标记层替换为视频标记层。 |
| ✅ Accordingly, video CoSwin replaces the tokenization layer of CoSwin (in Section 2.3 ) from 2D convolutional layers to 3D convolutional layers, which converts each 3D tube into one token. | ✅ 相应地，视频 CoSwin 将 CoSwin 的标记层（在 2.3 部分中）从 2D 卷积层替换为 3D 卷积层，将每个 3D 管转换为一个标记。 |
...
| ✅ Second, video CoSwin uses the 3D convolution-based patch merging operator instead of the 2D patch merging operator used in ( **Video swin transformer.** ). | ✅ 其次，视频 CoSwin 使用基于 3D 卷积的补丁合并算子，而不是 ( **Video swin transformer.** ) 中使用的 2D 补丁合并算子。 |
...
| ✅ We duplicate the 2D relative positional embedding matrix from the pre-trained CoSwin along the temporal dimension to initialize the 3D positional embedding matrix. | ✅ 我们沿时间维度从预先训练的 CoSwin 中复制 2D 相对位置嵌入矩阵，以初始化 3D 位置嵌入矩阵。 |
...
| ✅ To mitigate memory issues in the video training, we adopt the dynamic window size strategy, i.e., a relatively small window size in early stages of CoSwin , and large window sizes in its later stages. | ✅ 为了减轻视频训练中的内存问题，我们采用动态窗口大小策略，即在 CoSwin 的早期阶段使用相对较小的窗口大小，在后期阶段使用较大的窗口大小。 |
...
| ---- | ---- |
| ✅ For a checkpointed model component, e.g.  , multi-head attention, it reruns a forward pass during backward pass. | ✅ 对于检查点模型组件 e.g.  ，多头注意力，它在后向传递期间重新运行前向传递。 |
...
| ---- | ---- |
| ✅ In mixed-precision training, various operations are trained with different numerical precision ( i.e.  , float-32 or float-16). | ✅ 在混合精度训练中，各种操作以不同的数值精度（i.e. 、浮点型32或浮点型16）进行训练。 |
...
| ✅ As a vision foundation model, Florence can be directly used to predict if an image and a text snippet are semantically matched together in the task dataset. | ✅ Florence 作为视觉基础模型，可直接用于预测任务数据集中图像与文本片段是否在语义上匹配。 |
...
| ✅ We compute the feature embedding of the image for CoSwin and the feature embedding of the set of possible texts by the language encoder. | ✅ 我们计算 CoSwin 的图像的特征嵌入以及语言编码器可能的文本集的特征嵌入。 |
...
| ✅ On the two datasets, the input image resolution is quite low ( i.e.  ,  $32\times 32$  ). | ✅ 在这两个数据集上，输入图像分辨率相当低（i.e. ， $32\times 32$ ）。 |
...
| ---- | ---- |
| ✅ Florence can be easily adapted to support continual fine-tuning on target classification tasks. | ✅ Florence 可以轻松适应以支持对目标分类任务的持续微调。 |
...
| ✅ Our image encoder CoSwin -H is fine-tuned at the resolution of  $512\times 512$  with a batch size of  $8,192$  for  $10$  epochs. | ✅ 我们的图像编码器 CoSwin-H 在  $10$  时期以  $512\times 512$  的分辨率和  $8,192$  的批量大小进行微调。 |
...
| ✅ Our result is slightly worse than SOTA ( **Coatnet: Marrying convolution and attention for all data sizes.** ) , but their model and data scale are both  $3\times$  larger. | ✅ 我们的结果比SOTA ( **Coatnet: Marrying convolution and attention for all data sizes.** )稍微差一些，但是他们的模型和数据规模都比 $3\times$ 大。 |
...
| ✅ Zero-shot Florence matches or outperforms all prior zero-shot results on these two datasets. | ✅ 零样本 Florence 在这两个数据集上匹配或超越了所有先前的零样本结果。 |

| 【第3.5节，第3段】原文 | 【第3.5节，第3段】翻译 |
| ---- | ---- |
| ✅ For fine-tuning retrieval, we continuously train our language and text encoders on the target image-text pair data, as well as classification fine-tuning (shown in Section 3.3 ). | ✅ 对于微调检索，我们在目标图像-文本对数据上不断训练我们的语言和文本编码器，以及分类微调（如 3.3 部分所示）。 |
| ✅ We fine-tune our model with a batch size of  $3,072$  for  $12$  epochs. | ✅ 我们使用  $3,072$  的批次大小针对  $12$  个时期对模型进行微调。 |
...
| ✅ Compared with existing large-scale pre-trained models ( e.g.  , CLIP ( **Learning transferable visual models from natural language supervision.** ) , ALIGN ( **Scaling up visual and vision-language representation learning with noisy text supervision.** ) , Wu Dao 2.0 ( **https://gpt3demo.com/apps/wu-dao-20.** ) ), Florence is more desirable for object detection since its adaptation helps learn visual representation at the object level. | ✅ 与现有的大规模预训练模型（e.g. 、CLIP ( **Learning transferable visual models from natural language supervision.** )、ALIGN ( **Scaling up visual and vision-language representation learning with noisy text supervision.** )、Wu Dao 2.0 ( **https://gpt3demo.com/apps/wu-dao-20.** )）相比，Florence 更适合用于物体检测，因为它的适应性有助于学习物体级别的视觉表征。 |
...
| ✅ For Object365, we use the same input resolution of images ( i.e.  , the maximum image side  $1,333$  ) as the Multi-dataset Detection 111This work was ranked 1-st in the object detection track of ECCV 2020 Robust Vision Challenge.   ( **Simple multi-dataset detection.** ) for fine-tuning. | ✅ 对于 Object365，我们使用与多数据集检测 111This work was ranked 1-st in the object detection track of ECCV 2020 Robust Vision Challenge.   ( **Simple multi-dataset detection.** ) 相同的图像输入分辨率（ i.e.  、最大图像边  $1,333$  ）进行微调。 |
...
| ✅ For COCO, Object365 and zero-shot transfer benchmarks, we use mAP, i.e.  , average over multiple IoUs (  $0.5:0.05:0.95$  ). | ✅ 对于 COCO、Object365 和零样本传输基准，我们使用 mAP、i.e.  和多个 IoU 的平均值（ $0.5:0.05:0.95$ ）。 |
| ✅ For Visual Genome, we use AP50 at IoU threshold  $0.5$ . | ✅ 对于 Visual Genome，我们在 IoU 阈值  $0.5$  处使用 AP50。 |
| ✅ As we can see, Florence establishes new results in these main benchmarks of object detection. | ✅ 我们可以看到，Florence 在这些主要的物体检测基准测试中建立了新的成果。 |

#### 3.6.2 Zero-shot Transfer

| 【第3.6.2节，第1段】原文 | 【第3.6.2节，第1段】翻译 |
| ---- | ---- |
| ✅ Zero-shot object detection is more challenging than zero-shot classification, since neither object proposal classification nor location ( i.e.  , bounding box regression) in downstream tasks is seen during training. | ✅ 零样本物体检测比零样本分类更具挑战性，因为在训练期间既看不到下游任务中的对象提议分类，也看不到位置（i.e. ，边界框回归）。 |
...
| ✅ Using this approach, we can follow existing work on zero-shot image classification to zero-shot transfer in object detection, to evaluate the Florence for novel object recognition. | ✅ 使用这种方法，我们可以遵循现有的零样本图像分类工作，将零样本迁移到物体检测中，以评估 Florence 对新物体识别的效果。 |
...
| ✅ Specifically, we freeze the CoSwin backbones and pre-train the Dynamic Head on FLOD-9M by neglecting semantics from each object bounding box. | ✅ 具体来说，我们冻结 CoSwin 主干，并通过忽略每个对象边界框的语义在 FLOD-9M 上预训练 Dynamic Head。 |
...
| ✅ Table 7 shows that our Florence model effectively zero-shot transfers to these tasks. | ✅ 表 7 显示我们的 Florence 模型有效地对这些任务进行了零样本转移。 |
...
| ✅ Florence outperforms in  $7/11$  tasks over 5-shot fine tuning, and outperforms full-set fine-tuning on the “Packages” dataset, consisting of only  $26$  images for training. | ✅ Florence 在  $7/11$  任务中的表现优于 5 次微调，并且优于在仅由  $26$  图像组成用于训练的“Packages”数据集上的全集微调。 |
...
| ✅ Beyond replacing the image encoder with CoSwin -H of our Florence model on ( **An empirical study of training end-to-end vision-and-language transformers.** ) , we remove the weight decay on the text embedding layer and the modality-specific embedding. | ✅ 除了在 ( **An empirical study of training end-to-end vision-and-language transformers.** ) 上用 Florence 模型的 CoSwin-H 替换图像编码器之外，我们还删除了文本嵌入层和特定于模态的嵌入上的权重衰减。 |
| ✅ ITM and MLM are applied for VLP with  $43$  epochs with the image input size as  $384$  . | ✅ ITM 和 MLM 应用于  $43$  时期的 VLP，图像输入大小为  $384$ 。 |
...
| ---- | ---- |
| ✅ Although Florence is pre-trained on image-text pairs, it can be easily adapted to video tasks (shown in Section 2.6 ), such as text-video retrieval. | ✅ 尽管 Florence 是在图像-文本对上进行预训练的，但它可以轻松适应视频任务（如 2.6 部分所示），例如文本-视频检索。 |
| ✅ We expand the input 2D patch embeddings and positional embeddings to 3D so that the encoder can process video inputs, following ( **Vivit: A video vision transformer.** ). | ✅ 我们按照 ( **Vivit: A video vision transformer.** ) 将输入的 2D 块嵌入和位置嵌入扩展为 3D，以便编码器可以处理视频输入。 |
...
| ✅ Our attempt is a step towards building XYZ-code ( **A holistic representation toward integrative ai.** ) , an integrative AI system that makes progress toward human-like AI. | ✅ 我们的尝试是朝着构建 XYZ-code ( **A holistic representation toward integrative ai.** ) 迈出的一步，这是一个朝着类人 AI 迈进的综合 AI 系统。 |
