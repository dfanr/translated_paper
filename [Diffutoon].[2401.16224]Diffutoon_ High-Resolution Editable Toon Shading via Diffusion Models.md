...
| ✅ Our work is accompanied by the release of both the source code and example videos on Github 111Project page: https://ecnu-cilab.github.io/DiffutoonProjectPage/ . | ✅ 我们的工作伴随着在 Github 111Project page: https://ecnu-cilab.github.io/DiffutoonProjectPage/ 上源代码和示例视频的发布。 |
...
| ---- | ---- |
| ✅ Stylization : we leverage a personalized Stable Diffusion ( **High-resolution image synthesis with latent diffusion models. In Proceedings of the IEEE/CVF conference on computer vision and pattern recognition. 10684–10695.** ) model for anime stylization. | ✅ 风格化：我们利用个性化的稳定扩散 ( **High-resolution image synthesis with latent diffusion models. In Proceedings of the IEEE/CVF conference on computer vision and pattern recognition. 10684–10695.** ) 模型进行动漫风格化。 |
...
| ---- | ---- |
| ✅ Consistency enhancement : to enhance the temporal consistency, we employ several motion modules in our approach. | ✅ 一致性增强：为了增强时间一致性，我们在方法中采用了多个运动模块。 |
...
| ---- | ---- |
| ✅ Structure guidance : we extract the outline information from the input video and use a ControlNet model to retain the outline information during the generation process. | ✅ 结构指导：我们从输入视频中提取轮廓信息，并使用ControlNet模型在生成过程中保留轮廓信息。 |
...
| ---- | ---- |
| ✅ Colorization : we use another ControlNet model for colorization. | ✅ 着色：我们使用另一个 ControlNet 模型进行着色。 |
...
| ---- | ---- |
| ✅ As illustrated in the top part of Figure 1 , the main toon shading pipeline involves several key steps. | ✅ 如图 1 上部所示，主卡通着色管道涉及几个关键步骤。 |
...
| ✅ By replacing $\tau$ with $\tau^{\prime}$ in ( 3 ) and ( 4 ), we can obtain the estimated noise on the negative side $\overline{\boldsymbol{e}}_{t,-}(i)$. | ✅ 通过在（3）和（4）中用$\tau^{\prime}$替换$\tau$，我们可以得到负侧的估计噪声$\overline{\boldsymbol{e}}_{t,-}(i)$。 |
...
| ✅ The architecture of the editing branch is shown in the bottom part of Figure 1. | ✅ 编辑分支的架构如图 1 的底部所示。 |
...
| ---- | ---- |
| ✅ Stylization : we leverage the same Stable Diffusion model as that in the main toon shading pipeline. | ✅ 风格化：我们利用与主卡通着色管道中相同的稳定扩散模型。 |

| 【第3.2节，第4段】原文 | 【第3.2节，第4段】翻译 |
| ---- | ---- |
| ✅ Consistency enhancement : we use cross-frame attention and FastBlend ( **Duan et al. (2023b)  Zhongjie Duan, Chengyu Wang, Cen Chen, Weining Qian, Jun Huang, and Mingyi Jin. 2023b.   FastBlend: a Powerful Model-Free Toolkit Making Video Stylization Easier.   arXiv preprint arXiv:2311.09265 (2023).** ) to improve consistency. | ✅ 一致性增强：我们使用跨帧注意力和 FastBlend ( **Duan et al. (2023b)  Zhongjie Duan, Chengyu Wang, Cen Chen, Weining Qian, Jun Huang, and Mingyi Jin. 2023b.   FastBlend: a Powerful Model-Free Toolkit Making Video Stylization Easier.   arXiv preprint arXiv:2311.09265 (2023).** ) 来提高一致性。 |
...
| ---- | ---- |
| ✅ Colorization : the color is determined by the given prompt. | ✅ 着色：颜色由给定的提示决定。 |
...
| ✅ The prompt used for editing is “best quality, perfect anime illustration, a girl is dancing, smile, solo, orange dress , black hair , white shoes , blue sky ”. | ✅ 编辑所用的提示是“最佳品质、完美的动漫插图，一个女孩正在跳舞，微笑，独舞，橙色连衣裙，黑色头发，白色鞋子，蓝色天空”。 |
...
| ✅ We recorded the proportion of participant choices in Table 2. | ✅ 我们在表 2 中记录了参与者选择的比例。 |
...
| ✅ In the original video (Figure 2 a), a girl is dancing with fast movements, posing a significant challenge for each video processing method. | ✅ 在原始视频（图 2a）中，一个女孩正在快速跳舞，这对每种视频处理方法都提出了巨大的挑战。 |
...
| ✅ In the videos generated by DomoAI (Figure 2 e and Figure 2 f), there is missing content in the third frame, and the character’s movements in the fourth frame do not align with the original video. | ✅ 在 DomoAI 生成的视频中（图 2e 和图 2f），第三帧缺少内容，第四帧中角色的动作与原始视频不一致。 |
...
| ✅ Contrastingly, videos generated by Diffutoon (Figure 2 g and Figure 2 h) showcase the preservation of details such as lighting, hair, and pose, while maintaining a visual style closely aligned with anime aesthetics. | ✅ 相比之下，Diffutoon 生成的视频（图 2g 和图 2h）展示了对灯光、头发和姿势等细节的保留，同时保持了与动漫美学紧密一致的视觉风格。 |
...
| ✅ The outline video and the color video provide essential information for rendering a high-resolution video in Figure 2 h. | ✅ 图 2h 中的轮廓视频和彩色视频为渲染高分辨率视频提供了必要的信息。 |
...
| ✅ The rendered videos without each ControlNet model are shown in Figure 4 and Figure 5. | ✅ 图 4 和图 5 显示了不含各个 ControlNet 模型的渲染视频。 |
...
| ✅ As we mentioned in Section 3.2 , the reason is that AnimateDiff relies on a modified DDIM scheduler. | ✅ 正如我们在第 3.2 节中提到的，原因是 AnimateDiff 依赖于修改后的 DDIM 调度程序。 |
...
| ✅ These models are listed in Table 3. | ✅ 这些模型列于表 3 中。 |
